...
function ShopFrame_OnLoad()
    ShopFrameDescText:SetText("浏览并选择您喜欢的商品.");
...
function tws.processBalance(arg)
    ...
    if ex[2] and ex[2] ~= "" then
        tokenName = ex[2]
    end

    if ex[3] and tonumber(ex[3]) then
        tws.balance = tonumber(ex[3])
        ShopFrameBalance:SetText(tokenName..": " .. tws.balance)
    end
end
...
function EntriesSort()
    local oneindex = 1
    for key, value in pairs(tws.entries) do
        local idx = key
        local twoindex = 1
        for key1, value1 in pairs(tws.entries) do
            if twoindex > oneindex then
                if tws.entries[key1].id < tws.entries[idx].id then
                    idx = key1
                end
            end
            twoindex = twoindex + 1
        end
        tws.entries[key], tws.entries[idx] = tws.entries[idx], tws.entries[key]
        oneindex = oneindex + 1
    end
end
    ...
    if ex[2] then
        ...
        if ex[2] == "end" then
            ...
            if tws.totalPages > 1 then
                ShopFrameFramePageText:SetText("页 " .. tws.entryPage .. "/" .. tws.totalPages)
...
function tws.processBuyResult(arg)
    ...
    if rEx[2] then
        ...
        if rEx[2] == 'bagsfulloralreadyhaveitem' then
            twsprint("购买失败！你的背包可能已经满了！")
        end
        if rEx[2] == 'unknowndberror' then
            twsprint("未知错误！")
        end
        if rEx[2] == 'notenoughtokens' then
            twsprint("购买失败！交易货币不足！")
        end
        if rEx[2] == 'ok' then
            twsprint("购买完成！")
            ShopFrameCategoryButton_OnClick(tws.currentCategory)
        end
    end
    tws.getBalance()
end
...
function ShopFrameEntryButton_OnClick(id)
    ...
    if tws.balance < tws.entries[id].price then
        tws.AddButtonOnEnterTextTooltip(ShopFrameEntryFrameBuyButton, "你没有足够的货币购买这个物品!", "请访问：www.baidu.com购买代币.")
    end
